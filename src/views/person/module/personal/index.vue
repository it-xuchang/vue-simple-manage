      ...
      <div class="personal-content pull-right">
        <div class="personal-cont">
          <div class="top">
            ...
            <div class="top-cont">
              <div class="col-lg-8">
                <div class="imgIco"><img src="../../../../assets/img/asset-timg.png" width="60" height="28" alt=""></div>
                <div class="title"><span class="lab">继续学习</span> 程序语言设计 <span class="status">学习中</span></div>
                <div class="about"><span class="lab">正在学习</span> 使用对象 <span class="data">开始日期: 2020.01.01</span></div>
                <div class="rate">
                  <el-progress :text-inside="true" :stroke-width="15" status="success" :percentage="70"></el-progress>
                </div>
              </div>
              <div class="division"></div>
              <div class="col-lg-4 text-right">
                <a href="#" class="goLear"> 继续学习</a>
                <a href="#" class="evalu"> 课程详情</a>
              </div>
          ...
          <div class="my-course">
            <div class="title">
              ...
              <div class="tit-Item"><span><em class="active">按学习时间进行排序</em> <em>按加入时间进行排序</em></span><span><em class="active">全部</em><em>课程名称</em><em>课程类别</em></span></div>
            </div>
            <div class="cont">
              <div class="col-lg-6" v-for="item in personAllCourseDetail.datas" key="item">
                <div class="item">
                  <div class="imgIco"><img src="../../../../assets/img/asset-timg.png" width="60" height="28" alt=""></div>
                  <div class="title"><span class="lab">继续学习</span> {{item.courseBaseResult.courseName}} <span class="status">正在学习</span>
                  </div>
                  <div class="about" ><span class="lab">正在学习：</span> <a href="#" @click="handleVedioPlay(item.learningTeachPlan)">{{item.learningTeachPlan == null ? '未进行学习':item.learningTeachPlan.teachplanName}} </a></div>
                  <div class="time">开始日期: {{item.createTime}}</div>
                  <div class="rate">
                    <el-progress :text-inside="true" :stroke-width="20" :percentage="item.progressBar" status="success"></el-progress>
                  </div>
                  <div class="parting"></div>
                  <div class="butItem text-center"><a href="#" class="golearing">{{item.status}}</a> <a href="" @click="handleRouterQueryCourse(item.courseBaseResult.courseId)" class="classcom">课程详情</a></div>
                </div>
              </div>
              <div class="clearfix"></div>
              <!--分页栏-->
                  ...
                  <el-col span="24">
                    <div class="block">
                      <el-pagination
                        ...
                        :current-page="personAllCourseDetail.pageNum"
                        :page-sizes="[4, 8, 16, 20]"
                        :page-size="personAllCourseDetail.pageSize"
                        background="true"
                        layout="total, sizes, prev, pager, next, jumper"
                        :total="personAllCourseDetail.total">
                      </el-pagination>
                    </div>
                  </el-col>
                </el-row>
...
<script>
  ...
  import {queryPersonCourse} from '@/api/personal'
  export default {
    name: 'perosonalCourse',
    computed: {
      ...mapGetters([
        'name'
      ])
    ...
    created () {
      this.handleQueryPersonAllCourse();
    },
    methods:{
      handleRouterQueryCourse(id){
      ...
      handleSizeChange(val) {
        ...
        param.orderBy = this.pageParam.orderBy
        queryPersonCourse(param).then(response => {
          this.personAllCourseDetail = response.data.data
        }).catch(error => {
          this.$message(error)
        })
      },
      ...
      handleQueryPersonAllCourse(){
        ...
        queryPersonCourse(param).then(response => {
          ...
          console.log(this.personAllCourseDetail.datas)
        }).catch(error => {
          this.$message(error)
        })
      },
      handleVedioPlay(learningTeachPlan){
        let routeData = this.$router.resolve({
          path: '/video/play',
          query: {  courseId: learningTeachPlan.courseId ,teachplanId: learningTeachPlan.teachplanId}
        });
        window.open(routeData.href, '_blank');
      }
    }
